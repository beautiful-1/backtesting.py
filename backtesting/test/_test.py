...
from backtesting.test import EURUSD, GOOG, SMA, btcusdt
...
"""
这是Python中的装饰器，用于创建上下文管理器。上下文管理器用于在进入和退出特定代码块时执行必要的操作。

使用这个上下文管理器，您可以在一个特定的代码块内临时切换到不同的目录，执行您需要的操作，然后在退出该代码块后，工作目录将自动恢复到之前的状态，以避免对其他部分的代码产生影响。
这对于需要操作不同目录的任务非常有用，例如文件操作、数据处理等。
"""
...
def chdir(path):
    """
    这一行保存当前工作目录（当前目录）的路径到变量 cwd 中，以便在后面可以回到这个目录。
    """
    cwd = os.getcwd()
    """
    这一行将当前工作目录更改为传递给上下文管理器的 path。这就实现了切换工作目录的目的。
    """
    os.chdir(path)
    try:
        """
        yield语句暗示了一个上下文管理器的分隔点。它允许在进入和退出上下文时执行额外的操作。在这里，yield允许进入被切换的工作目录
        """
        yield
    finally:
        """
        在finally块中，代码将当前工作目录恢复为之前保存的原始工作目录 cwd。这确保了不管是否发生异常，都会返回到原始工作目录。
        """
        os.chdir(cwd)
...
class SmaCross(Strategy):
    ...
    def init(self):
        ...
        self.macd = self.I(self.macd_f, self.data.Close, "macd", plot=False)
        self.signal = self.I(self.macd_f, self.data.Close, "signal", plot=False)
        self.hist = self.I(self.macd_f, self.data.Close, "hist", scatter=True, plot=False)
...
class MyStrategy(Strategy):
    def init(self):
        ...
        self.signal = self.I(self.macd_f, self.data.Close, "signal", plot=False)
        self.hist = self.I(self.macd_f, self.data.Close, "hist", scatter=True, plot=False)
        self.macd_long_single = 0
        self.macd_short_single = 0

    def next(self):
        if not self.position:
            if self.macd_long_single > self.hist[-1] > self.hist[-2] and self.hist[-2] < self.hist[-3] < self.macd_long_single:
                self.position.close()
                self.buy()

            if self.macd_short_single < self.hist[-1] < self.hist[-2] and self.hist[-2] > self.hist[-3] > self.macd_short_single:
                self.position.close()
                self.sell()
        else:
            if self.data_close[0] > self.take_profit_price:  # 触发止盈
                self.close()
            elif self.data_close[0] < self.stop_loss_price:  # 触发止损
                self.close()
...
class TestBacktest(TestCase):
    ...
    #     """
    #     它使用self.assertRaises方法来测试是否会引发AttributeError异常。
    #     具体来说，它期望在运行bt.run(foo=3)时引发AttributeError异常。
    #
    #     AttributeError是一种Python中的异常类型，通常表示在尝试访问或操作对象的属性时出现问题。
    #     在这种情况下，测试似乎是为了检查bt.run方法是否接受不支持的参数foo=3并引发异常。
    #
    #     bt.run(foo=3)：此行调用了bt对象的run方法，并传递了一个名为foo的参数，其值为3。
    #     """
    ...
    #     end_time = datetime.date(2017, 5, 1)
    #     cur_df = df[start_time:end_time]
    #     bt = Backtest(cur_df, SmaCross)
    #     bt.run()
    #     # 参数优化
    #     bt.optimize(fast=4, slow=[6, 8])
    #     bt.plot(plot_drawdown=True, open_browser=True)
    #     self.assertTrue(df.equals(EURUSD))

    def test_btc_data(self):
        df = btcusdt.copy()
        start_time = datetime.date(2023, 6, 1)
        end_time = datetime.date(2023, 7, 1)
        cur_df = df[start_time:end_time]
        bt = Backtest(cur_df, SmaCross)
        bt.run()
        # 参数优化
        # bt.optimize(fast=4, slow=[6, 8])
        bt.plot(plot_drawdown=True, open_browser=True)
        self.assertTrue(df.equals(btcusdt))
